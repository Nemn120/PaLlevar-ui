...
export class AttendOrderDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.orderDetailList = this.orderDetailList.filter(
      (x) => x.status == "Pendiente"
    );
    this.dataSource = new MatTableDataSource<OrderDetailBean>(
  ...
  atenderPedido(): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.orderService.saveAttendOrder(this.data).subscribe(
          (data) => {
            ...
            if (this.orderDetailList.length == numSelected.length)
